...
const controller = require("../controllers/secondpp.controller");
module.exports = function(app) {
  ...
  app.get('/second_installments', async (req, res) => {
    try {
        const installments = await Secondpp.findAll(); 
    ...
    app.get('/secondpp/unpaid', controller.getAllUnpaidSecondpp);
  // Route pour récupérer un installment par ID
app.get('/sinstallments/:installmentID', async (req, res) => {
  const installmentID = req.params.installmentID;

  try {
      const secondpp = await Secondpp.findByPk(installmentID); // Récupérer l installment par son ID depuis la base de données

      // Vérifier si l installment existe
      if (secondpp) {
          res.status(200).json(secondpp); // Renvoyer l installment sous forme de réponse JSON
      } else {
...
app.put('/sinstallments/up/:installmentId', async (req, res) => {
  const installmentId = req.params.installmentId;
  ...
  app.delete('/sinstallments/del/:installmentId', async (req, res) => {
    const installmentId = req.params.installmentId;
    try {
  ...
  app.post('/sinstallment', async (req, res) => {
      try {
          const secondpp = await Secondpp.create({
            second_installmentDate: req.body.second_installmentDate,
            second_installmentPrice: req.body.second_installmentPrice,
            second_installmentStatus: req.body.second_installmentStatus,
